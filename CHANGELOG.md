...
### Fixed
- **The setup wizard could silently re-point the site's default language.** Its redirect fires whenever the completion flag is missing, so a site configured by CLI (or any site where the flag was lost) sends the next admin into the wizard, and finishing a step promotes the chosen language to default. Every post and term tagged in the OLD default then becomes a translation of a language nobody browses: it drops out of the default pool, taking product categories — and therefore product queries — with it. On a live site this reads as "the dashboard shows 0 cars and the front end is broken". The wizard now only sets the default when that is still an open question (no default yet, or nothing tagged in the current one); deliberate changes remain available on the Languages screen and via WP-CLI.
- **The switcher linked a language's front page by slug** (`/en/home/`) instead of its root (`/en/`), costing a 301 on every switch and putting a second URL for the same view into hreflang and the sitemap.
- **The switcher's default-language option kept the foreign prefix.** On an untranslated singular, resolving the default language went through `get_permalink()`, which now answers in the language being browsed — so "switch to Georgian" from `/en/product/…` linked straight back to `/en/product/…`. The switcher pins the prefix to the language it is switching *to*.
- **Switching language from a WooCommerce account endpoint dropped the endpoint.** From `/en/my-account/car-invoices/` all three targets pointed at the account root. Any path the request carries beyond the queried post's own permalink is now re-attached to the target URL.
- **Links out of an untranslated post dropped the visitor back into the default language.** A post with no translation in the language being browsed is still served under that prefix, but `PostLinkFilter` kept linking with the post's own language — so every product link on an /en/ page, and every car and invoice row in the dealer dashboard, pointed at the Georgian URL. Such posts now link within the language being browsed. Posts that do have a real translation are unaffected: asking for a specific post's permalink still returns that post's own URL, and the admin permalink preview is untouched.
